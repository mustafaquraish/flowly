...
class HtmlExporter:
    """Exports a FlowChart or MultiFlowChart to a standalone interactive HTML player.
    ...
    def _extract_body_content(html: str) -> str:
        ...
        body_content = re.sub(
            r'<script\s+src=["\']app\.js["\']\s*></script>', "", body_content
        )
    ...
    def _extract_head_content(html: str) -> str:
        ...
        head_content = re.sub(
            r'<link\s+rel=["\']stylesheet["\']\s+href=["\']styles\.css["\']\s*/?\s*>',
            "",
            head_content,
        )
    ...
    @staticmethod
    def to_html(flowchart: FlowChart | MultiFlowChart) -> str:
        """Returns the complete standalone HTML string with bundled flowplay.

        Accepts either a single FlowChart or a MultiFlowChart.
        """
        # Convert chart to dictionary and JSON
        if isinstance(flowchart, MultiFlowChart):
            data = JsonSerializer.multi_to_dict(flowchart)
            name = flowchart.name
        else:
...
{app_js}
    ...
    @staticmethod
    def save(flowchart: FlowChart | MultiFlowChart, filename: str):
